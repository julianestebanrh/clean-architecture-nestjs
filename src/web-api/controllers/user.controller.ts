import { BadRequestException, Body, Controller, Get, NotFoundException, Param, ParseUUIDPipe, Post, Put, UseGuards, UseInterceptors } from '@nestjs/common';
...
import { CacheInvalidatorInterceptor } from '@/infrastructure/cache/cache-invalidate.interceptor';
...
export class UserController {
  ...
  async createUser(@Body() createUserDto: CreateUserDto) {
    ...
    const response =  await this.commandBus.execute(new CreateUserCommand(name, email, password));
  ...
  async updateUser(@Param('id', ParseUUIDPipe) id: string, @Body() updateUserDto: CreateUserDto) {
    ...
    const response = await this.commandBus.execute(new UpdateUserCommand(id, name, email, password));
    if(response.isFailure) {
